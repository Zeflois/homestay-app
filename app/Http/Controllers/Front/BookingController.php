...
class BookingController extends Controller
{
    ...
    function midtransAction(Request $request)
    {
        // $provider = new PayPalClient;
        // $provider -> setApiCredentials(config('paypal'));
        // $paypalToken = $provider->getAccessToken();
        ...
        if ($request->result_type == 'pending') {
            $order_no = time();
            ...
            $obj->customer_id = Auth::guard('customer')->user()->id;
            $obj->order_no = $order_no;
            $obj->transaction_id = $dataMidtrans->order_id;
            $obj->pdf_url = $dataMidtrans->pdf_url;
            $obj->payment_method = 'Midtrans';
            $obj->paid_amount = $dataMidtrans->gross_amount;
            $obj->booking_date = date('d/m/Y');
            $obj->status = $request->result_type;
            $obj->save();
            ...
            $message = 'You have made an order for hotel booking. The booking information is given below: <br>';
            $message .= '<br>Order No: ' . $order_no;
            $message .= '<br>Transaction Id: ' .$dataMidtrans->order_id;
            $message .= '<br>Payment Method: PayPal';
            $message .= '<br>Paid Amount: ' . $dataMidtrans->gross_amount;
            $message .= '<br>Booking Date: ' . date('d/m/Y') . '<br>';
